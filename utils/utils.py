...
def check_permissions(required_permission='admin'):
    def real_decorator(func):
        ...
        def wrapped(update: Update, context: CallbackContext, *args, **kwargs):
            ...
            if update.callback_query:
                text = f'"{required_permission}"为非管理员用户禁用权限'
                update.callback_query.answer(text, show_alert=True, cache_time=30)
            elif update.message:
                update.message.reply_html(f'<code>[{required_permission}]</code> 为非管理员用户禁用权限')
...
def failwithmessage(func):
    ...
    def wrapped(update: Update, context: CallbackContext, *args, **kwargs):
        try:
            ...
            text = '处理时发生错误 {} (<code>{}()</code>): <code>{}</code>'.format(
    ...
    def wrapped(update: Update, context: CallbackContext, *args, **kwargs):
        try:
            ...
            if 'not modified' not in str(err).lower():
                ...
                update.callback_query.answer('没什么可刷新')
...
def failwithmessage_job(func):
    ...
    def wrapped(context: CallbackContext, *args, **kwargs):
        try:
            ...
            text = f'#{context.bot.username} exception: 运行作业时发生错误 ' \
