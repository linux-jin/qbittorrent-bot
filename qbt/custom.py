...
NEW_ATTRS = {
    ...
    'auto_tmm_string': lambda t: '是' if t['auto_tmm'] else '否',
}
...
TORRENT_STRING = """<code>{name_escaped}</code>
  ...
  <b>状态</b>: {state_pretty}
  <b>大小</b>: {size_pretty}
  <b>下载/上传速度</b>: {dl_speed_pretty}/s, {up_speed_pretty}/s
  <b>下载速度限制</b>: {dl_limit_pretty}
  <b>用户</b>: {peers}已连接({peers_total}在队伍中)
  <b>种子</b>: {seeds}已连接({seeds_total} 在队伍中)
  <b>种子（替代值）</b>: {num_seeds}已连接({num_complete}在队伍中)
  <b>连接用户</b>: {num_leechs}已连接({num_incomplete}在队伍中)
  <b>连接数</b>: {nb_connections}
  <b>分享率</b>: {share_ratio_rounded} (最大: {max_ratio})
  <b>已下载时间</b>: {eta_pretty}
  <b>剩余时间</b>: {time_elapsed_pretty}
  <b>分类</b>: {category}
  <b>强制开始</b>: {force_start_pretty}
  <b>标签</b>: <code>{tags}</code>
  <b>自动种子管理</b>: {auto_tmm_string}
