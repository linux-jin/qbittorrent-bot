...
def on_add_or_remove_tags_command(update: Update, context: CallbackContext):
    ...
    if not hash_match:
        update.message.reply_text("返回种子信息 (必须包含种子hash)")
        return
    ...
    if action == "+":
        text = f"Tags添加到<b>{torrent.name_escaped}</b>: <code>{tags_list_str}</code> " \
               f"[<a href=\"{torrent.info_deeplink}\">info</a>]"
        torrent.add_tags(tags_list)
    else:
        text = f"Tags移除<b>{torrent.name_escaped}</b>: <code>{tags_list_str}</code> " \
               f"[<a href=\"{torrent.info_deeplink}\">info</a>]"
