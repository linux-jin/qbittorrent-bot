...
def on_torrents_list_selection(update: Update, context: CallbackContext):
    ...
    update.message.reply_html(f"列出<code>{qbfilter}</code>的种子(可能会花一点时间):")
    ...
    if not torrents:
        update.message.reply_html('没有<i>{}</i>的种子'.format(qbfilter))
        return
...
updater.add_handler(
    ...
    bot_command=[BotCommand(c, f"筛选 {c} 种子") for c in TORRENTS_CATEGORIES],
)
updater.add_handler(
    CommandHandler(["available_filters", "af"], on_available_filters_command),
    bot_command=[BotCommand("available_filters", "显示可以用来列出种子数据的命令列表")],
)
