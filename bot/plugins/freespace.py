...
def on_freespace_command(update: Update, context: CallbackContext):
    ...
    text = f"{drive_free_space} 剩余，默认保存路径: <code>{qb.save_path}</code>"

    update.message.reply_html(text)


updater.add_handler(CommandHandler(["space", "freespace"], on_freespace_command), bot_command=BotCommand("freespace", "剩余磁盘空间"))
