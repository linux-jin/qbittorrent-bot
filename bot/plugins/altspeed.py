...
def change_alternative_limits(update: Update, context: CallbackContext):
    ...
    if re.search(r'^[!/]altdown$', update.message.text, re.I):
        ...
        update.message.reply_markdown('选择替代的下载速度', reply_markup=reply_markup)

        return

    if not context.args:
        update.message.reply_text("必须在命令后提供速度 (如 kb/s)")
        return
    ...
    if not kbs.isdigit():
        update.message.reply_text('请以整数形式传递以 kb/s 为单位的替代速度限制')
        return
...
updater.add_handler(CommandHandler(['altdown', 'altup'], change_alternative_limits), bot_command=[
    BotCommand("altdown", "设置新的下载速度"),
    BotCommand("altup", "设置新的上传速度"),
])
