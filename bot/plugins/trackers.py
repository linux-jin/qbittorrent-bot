...
def on_remove_dead_trackers_command(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(f"移除 {removed_trackers} trackers 从 {affected_torrents} 种子")


# updater.add_handler(CommandHandler(['removedeadtrackers', 'rdt'], on_remove_dead_trackers_command), bot_command=BotCommand("removedeadtrackers", "删除死种"))
